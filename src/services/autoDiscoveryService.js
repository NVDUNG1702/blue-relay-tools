...
import ICloudDetectionService from './iCloudDetectionService.js';
...
class AutoDiscoveryService {
    ...
    // Tự động detect thông tin device
    async detectDeviceInfo() {
        try {
            ...
            // Lấy thông tin iCloud từ nhiều nguồn khác nhau
            ...
            for (const host of commonHosts) {
                for (const port of commonPorts) {
                    ...
                    try {
                        ...
                        // Server không phản hồi hoặc không phải Blue Relay server
                        continue;
                    }
                }
            }

            // Thử discovery endpoint trên các server tìm được
            for (const server of discovered) {
                try {
                    const discoveryResponse = await axios.post(`${server.url}/api/device-discovery/discover`, {
                        device_info: this.deviceInfo,
                        device_capabilities: this.deviceInfo.capabilities
                    }, {
                        timeout: 5000
                    });
    ...
    async connectToBestServer() {
        try {
            ...
            // Chọn server tốt nhất (có discovery support)
            ...
            if (bestServer.discovery) {
                ...
                logger.info('✅ Server configuration received:', {
                    server_url: bestServer.url,
                    device_id: this.serverConfig.device_info.device_id,
                    connection_id: this.serverConfig.device_info.connection_id
                });

                return this.serverConfig;
            } else {
                // Fallback cho server không có discovery
                this.currentServer = bestServer;
                this.serverConfig = {
                    server_info: {
                        endpoints: {
                            socket: bestServer.url,
                            api: `${bestServer.url}/api`
                        }
                    },
                    device_info: {
                        device_id: `mac-relay-${Date.now()}`,
        ...
        try {
            if (this.currentServer && this.serverConfig) {
                ...
                const response = await axios.put(
                    `${this.currentServer.url}/api/device-discovery/config/${deviceId}`,
                    {
                        device_info: { ...this.deviceInfo, ...newInfo },
